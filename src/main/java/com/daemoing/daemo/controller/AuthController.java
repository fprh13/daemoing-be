...
import com.daemoing.daemo.service.AuthService;
...
public class AuthController {
    ...
    public ResponseEntity<ResponseDto> delete(@RequestHeader("Authorization") String requestAccessToken, @PathVariable String loginId) {
    ...
    public ResponseEntity<?> validate(@RequestHeader("Authorization") String requestAccessToken) {
        if (!authService.validate(requestAccessToken)) {
            return ResponseEntity
                    .status(OK)
                    .body(ResponseDto.success(OK,null)); // 재발급 필요X
        } else {
    ...
    public ResponseEntity<?> reissue(@CookieValue(name = "refresh-token") String requestRefreshToken,
        ...
        if (reissuedTokenDto != null) { // 토큰 재발급 성공
            ...
            return ResponseEntity
                    .status(UNAUTHORIZED)
                    .header(HttpHeaders.SET_COOKIE, responseCookie.toString())
                    .body(ResponseDto.fail(UNAUTHORIZED,"토큰이 재발급 실패, 다시 로그인 하세요."));
        }
    }
}
